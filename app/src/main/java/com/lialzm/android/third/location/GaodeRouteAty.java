...
import com.lialzm.android.adapter.BusResultListAdapter;
import com.lialzm.android.third.location.route.DriveRouteDetailActivity;
import com.lialzm.android.third.location.route.WalkRouteDetailActivity;
 ...
 */
public class GaodeRouteAty extends BaseActivity implements AMap.OnMapClickListener,
    ...
    protected void onCreate(Bundle bundle) {
        ...
        locationWrapper.setOnLocationFinishListener(new GaodeLocation.OnLocationFinish() {
            ...
            public void locationFinish(LocationInfo location) {
                ...
                if (LocationUtils.getDistance(mStartPoint.getLatitude(), mStartPoint.getLongitude(), mEndPoint.getLatitude(), mEndPoint.getLongitude()) < 1) {
                    onWalkClick(new View(GaodeRouteAty.this));
                } else {
                    onDriveClick(new View(GaodeRouteAty.this));
                }
            }
    ...
    private void setfromandtoMarker() {
        aMap.addMarker(new MarkerOptions()
                .position(LocationUtils.convertToLatLng(mStartPoint))
                .icon(BitmapDescriptorFactory.fromResource(R.drawable.start)));
        aMap.addMarker(new MarkerOptions()
                .position(LocationUtils.convertToLatLng(mEndPoint))
    ...
    public void searchRouteResult(int routeType, int mode) {
        if (mStartPoint == null) {
            CommonUtil.showToast(mContext, "定位中，稍后再试...");
            return;
        }
        if (mEndPoint == null) {
            CommonUtil.showToast(mContext, "终点未设置");
        }
        showProgressDialog();
        ...
        if (errorCode == 1000) {
            if (result != null && result.getPaths() != null) {
                if (result.getPaths().size() > 0) {
                    ...
                    CommonUtil.showToast(mContext, R.string.no_result);
                }
            } else {
                CommonUtil.showToast(mContext, R.string.no_result);
            }
        } else {
            CommonUtil.showToast(this.getApplicationContext(), String.valueOf(errorCode));
        }
    }
        ...
        if (errorCode == 1000) {
            if (result != null && result.getPaths() != null) {
                if (result.getPaths().size() > 0) {
                    ...
                    String des = LocationUtils.getFriendlyTime(dur) + "(" + LocationUtils.getFriendlyLength(dis) + ")";
                    ...
                    CommonUtil.showToast(mContext, getString(R.string.no_result));
                }
    ...
    public void onWalkRouteSearched(WalkRouteResult result, int errorCode) {
        ...
        if (errorCode == 1000) {
            ...
            CommonUtil.showerror(this.getApplicationContext(), errorCode);
        }
    }
